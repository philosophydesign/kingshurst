...
if (empty($_SESSION['philosri_nonce'])) {
	$_SESSION['philosri_nonce'] = array('blank'=>'blank');
}
...
// 		echo 'W: '.$what.'<br>';
	...
	function make_form_html ($form, $args=array()) {
		...
		foreach ($form->fields as $f) {
			#echo $f->options.'<br>';
			$type = $f->type;
			$label = ($f->mandatory) ? $f->label.' '.$this->mandatoryMarker : $f->label;
			
			$o = (!empty($f->options)) ? $this->opt2arr($f->options) : array();
		...
		if (!empty($args['wrap'])) {
			...
			$html = '<form method="post" action="'.$action.'" class="'.$prefix.'">
						'.$html.'
